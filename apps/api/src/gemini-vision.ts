...
export class GeminiVisionProvider implements VisionProvider {
  ...
  async analyzeSelection(dataUrl: string): Promise<ObjectDescription> {
    return normalizeObjectDescription(await this.generate(PROMPT + FIELD_CONFIDENCE_PROMPT, [dataUrl]));
  }
  ...
  private async generate(prompt: string, images: string[]): Promise<unknown> {
    const send = () => fetch(`https://generativelanguage.googleapis.com/v1beta/models/${this.model}:generateContent?key=${encodeURIComponent(this.apiKey)}`, {
      ...
      body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: prompt }, ...images.map((image) => ({ inlineData: imagePart(image) }))] }], generationConfig: { responseMimeType: 'application/json', temperature: 0.2 } }),
    });
